...
#include<algorithm>
#include<random>
//#include "/home/abouchakra/MyLibrary/randnumgen.h"
//#include "/home/abouchakra/MyLibrary/individualCRP-roundProb.h"

//#include "randnumgenC.h"
...
//    else{
//         loca="/Users/abouchakra/Documents/projects/Collective-risk/CR-round-prob-program/CRP-roundProb-program/data/";
//    }
	string nameF0 ("");
	string nameF1 ("");
	...
	char *filN3 = new char[(nameF.length()+205)];
	...
	for(int i=0;i<inD;i++)
	{
		...
		popN0[i].SetEndowment(indEndowment);
...
//                //cout<<probOPT[i]<<" "<< prob[i]<<" "<< target<<" "<< totalA<<" "<< gamma[i]<<" "<< double(gInd[0])<<"\n";
            ...
            double probOutput=diffFNs(probOPT, prob, target, totalA, gamma, double(gInd));
...
void onegeneration (std::vector<Individual> &popN0, int const& roundN, int const& inD, int const& gInd, double const& selectionCo,int const& gameNum,double const& prob, int const& probOPT,double  gamma,double *totalC0,double *totalE0,double *genC0, string outOpt, int const & randRound,std::mt19937_64 & rng,std::uniform_real_distribution<double> & uniR, std::uniform_int_distribution<int> & uni2)
{
	...
	{
        onegame(popN0, roundN, inD, gInd, prob, probOPT, gamma,totalC0,totalE0,genC0,outOpt,randRound,rng, uniR,uni2);
	}
